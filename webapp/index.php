<?php
// index.php — Premium Food WebApp (PHP Version 6.2)
?>
<!DOCTYPE html>
<html lang="uz">
<head>
...
<body>
    <div id="ls">
        ...
        <div style="margin-top:20px; font-weight:800; opacity:0.3; font-size:0.9rem">v6.2 RELOADED ✅</div>
    </div>
    ...
    <main id="v-cart" class="view">
        ...
        <div id="cart-empty" class="hidden" style="text-align:center;padding:60px 0;opacity:0.6;font-size:1.2rem">🛒 Bo'sh</div>
        <div id="cart-foot" class="hidden">
            <textarea id="note" placeholder="Izoh..." style="width:100%;padding:18px;border:none;background:var(--sec);border-radius:20px;margin:20px 0;min-height:90px;font-size:1rem;"></textarea>
    ...
    <main id="v-prof" class="view">
        <div class="hero">
            ...
            <div id="u-debug" style="opacity:0.6;font-size:0.8rem;margin-top:8px">v6.2 FIX</div>
        </div>
        <div style="display:flex; flex-direction:column;">
            <div class="row-item"><span>📞 Telefon:</span><span id="u-phone" style="color:var(--p)">...</span></div>
            <button class="sync-btn" onclick="app.sync(true)">Yangilash 🔄</button>
            <div id="final-log" style="font-size:0.6rem; color:#aaa; text-align:center; margin-top:25px">Connecting...</div>
        </div>
    </main>
    ...
    <script>
        class PremiumApp {
            ...
            rI(id='all') {
                ...
                l.forEach(i=>{
                    const d=document.createElement('div'); d.className='card'; const q=this.cart[i.id]?.q||0;
            ...
            tab(t){
                ...
                document.querySelectorAll('.nav-tab').forEach(n=>n.classList.remove('active')); document.getElementById(`n-${n.id}`).classList.remove('active'); // Wait, minor fix here
